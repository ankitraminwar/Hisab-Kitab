...
const ACCOUNT_TYPES: { key: AccountType; label: string; icon: string; color: string }[] = [
  ...
  { key: 'investment', label: 'Investment', icon: 'trending-up', color: '#06B6D4' },
];

const ACCOUNT_COLORS = ['#22C55E', '#3B82F6', '#8B5CF6', '#F43F5E', '#F97316', '#06B6D4', '#EAB308', '#EC4899'];
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <Card style={styles.totalCard} glow>
          ...
          <Text style={[styles.totalAmount, { color: totalBalance >= 0 ? COLORS.textPrimary : COLORS.expense }]}>
        ...
        {accounts.map(account => (
          <AccountCard key={account.id} account={account} onDelete={async () => {
      ...
      <AddAccountModal
        ...
        onSave={() => { loadAccounts(); setShowAdd(false); }}
      />
    </SafeAreaView>
  );
}
    ...
    <Card style={styles.accountCard}>
      <View style={styles.accountRow}>
        ...
        <View style={styles.accountInfo}>
          ...
          <Text style={styles.accountType}>{typeInfo?.label || account.type} • {account.currency}</Text>
        </View>
...
const AddAccountModal: React.FC<{ visible: boolean; onClose: () => void; onSave: () => void }> = ({ visible, onClose, onSave }) => {
    ...
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      <View style={mStyles.overlay}>
        <View style={mStyles.sheet}>
          ...
          <TextInput value={name} onChangeText={setName} placeholder="Account name (e.g. HDFC Savings)" placeholderTextColor={COLORS.textMuted} style={mStyles.input} />
          <TextInput value={balance} onChangeText={setBalance} keyboardType="numeric" placeholder="Opening balance (₹)" placeholderTextColor={COLORS.textMuted} style={mStyles.input} />

          <Text style={mStyles.label}>Color</Text>
          <View style={mStyles.colorRow}>
            {ACCOUNT_COLORS.map(c => (
              <TouchableOpacity key={c} onPress={() => setColor(c)}
                style={[mStyles.colorDot, { backgroundColor: c, borderWidth: color === c ? 3 : 0, borderColor: '#fff' }]} />
            ))}
          </View>
...
const mStyles = StyleSheet.create({
  ...
  typeChip: {
    ...
    borderWidth: 1, borderColor: COLORS.border, backgroundColor: COLORS.bgElevated, marginRight: 8,
  },
...
const styles = StyleSheet.create({
  ...
  totalLabel: { ...TYPOGRAPHY.label, color: COLORS.textMuted, textTransform: 'uppercase' },
  totalAmount: { fontSize: 36, fontWeight: '800', letterSpacing: -1, marginVertical: 6 },
  ...
  accountType: { ...TYPOGRAPHY.caption, color: COLORS.textMuted, marginTop: 2 },
  ...
  defaultText: { ...TYPOGRAPHY.caption, color: COLORS.primary, fontWeight: '600', fontSize: 10 },
  deleteBtn: { padding: 4 },
});
